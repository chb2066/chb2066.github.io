...
SAM에서 가져갈 것은 "**과제를 프롬프트 가능하게 정의하면 학습하지 않은 태스크로 확장된다**"는는 발상이다.

고정된 클래스 집합을 분할하도록 학습하면 그 집합 밖으로 못 나간다. 하지만 "**무엇을 자를지 지정받아 자른다**"로로 과제를 정의하면, 지정하는 방법을 바꾸는 것만으로 새 태스크가 된다.
