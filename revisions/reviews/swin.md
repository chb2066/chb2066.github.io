...
#### 주요 전략
1. 윈도우 안에서만 self-attention을 계산해 복잡도를 이미지 크기에 선형으로 축소함.
2. 층마다 윈도우를 이동시키는 shifted window로 윈도우 간 연결 확보함.

#### 핵심

1. **ViT는 연산량이 커서 hidden node 수를 키우는 데 부담이 있었는데, Swin은 연산량이 작아서 키울 수 있음.** 같은 연산량 기준으로 한 픽셀의 정보를 더 많은 노드에 저장할 수 있으므로, 픽셀에 대한 정보를 더 정확하게 뽑음.
2. **기존에는 16×16 패치를 써서 한 패치가 받을 수 있는 정보가 제한적이었음.** 4×4, 8×8 같은 작은 패치를 쓰면 각 패치가 더 많고 다양한 정보를 얻어서 세밀한 특징이 나옴.
3. 아래는 1, 2를 가능하게 하는 방법론임.
...
- LN: Layer Normalization. 하나의 입력 벡터(토큰)에 대한 정규화임.
...
여기서 옮겨갈 만한 발상은 "**제한을 걸어 비용을 낮추고, 그 제한을 층마다 어긋나게 해서 손실을 메운다**"는는 골격이다. 한 층에서 잃은 연결을 다음 층의 다른 분할이 복구한다.
