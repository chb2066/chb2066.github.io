...
## 주요 전략
1. teacher와 student에 서로 다른 크기의 crop을 주는 multi-crop으로 부분에서 전체를 예측함.
2. centering과 sharpening을 짝지어 반대 방향으로 작용시켜 collapse 방지함.
...
- Collapse를 방지함. 라벨이 있다면 역전파를 통해 가중치가 한쪽에 쏠리지 않게 정렬되지만, 라벨이 없으면 한쪽으로만 쏠릴 수 있음.
...
- student는 gradient로 지속적으로 학습함. `Loss = -p₂ log p₁`
- teacher는 student의 파라미터를 **EMA로 전달받아** 갱신됨.
...
- `H`는 전체 클래스에 대해 각 정답값과 `log Ps`(예측값)를 곱한 것임. 이것을 모두 더해 `H`가 출력됨.
- **정답을 맞히는 예측값이 커질수록 전체 값이 작아짐.** 따라서 역전파에서 loss를 줄이는 방향이 곧 예측값을 키우는 방향임.
...
1. **self-attention map에서 객체의 경계와 배경이 분리됨.** 라벨 학습 없이 그렇게 됨.
2. DINO가 학습한 특징은 분할 성능이 좋아서 **k-NN만 써도 잘 됨.**
