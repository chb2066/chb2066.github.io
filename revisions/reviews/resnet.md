...
2. 원본 입력 `x`가 직접 다음 레이어로 전달되어 **정보 손실이 최소화됨.**
3. 모델이 `F(x)`만 수정하면 되므로, 입력 `x`를 통째로 최적화하는 것이 아니라 **부족한 부분만 보정**할 수 있음.
...
LoRA, adapter, ControlNet, adaLN-Zero가 전부 같은 골격이다. "**zero-init 잔차 가지로 새 능력을 붙인다**"는는 패턴은 이 논문에서 시작해서 지금도 계속 재사용되고 있다.
