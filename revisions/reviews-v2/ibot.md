...
> 📄 [**iBOT: Image BERT Pre-Training with Online Tokenizer**](https://arxiv.org/abs/2111.07832) · ICLR 2022 · Zhou, Wei, Wang et al.

## Abstract

**문제**
BERT식 masked modeling을 이미지로 옮기려면 **시각 토크나이저**가 필요한데, 언어와 달리 이미지는 연속적이라 의미 단위를 뽑기 어려움
기존 방법은 픽셀을 그대로 목표로 삼거나 미리 학습한 토크나이저를 쓰는데, 둘 다 **다단계 파이프라인**을 강요함

**해결책**
EMA teacher를 **온라인 토크나이저**로 삼아 MIM과 함께 학습 → 토크나이저 사전학습 단계가 사라짐
...
- 기존 DINO가 cls 토큰만 썼다면 iBOT은 **patch loss를 추가**해 마스킹된 패치를 맞히게 함
...
![Figure 2](/img/ibot/f2.png)

**시각 토크나이저가 어려운 이유**
- BERT류의 성공은 언어를 **의미 있는 단위로 토큰화**할 수 있다는 데 기댐 (WordPiece 같은 것)
...
- teacher가 momentum update로 MIM과 **공동 최적화**됨 → 전처리 단계의 별도 학습이 필요 없음
...
- **online tokenizer가 별도 pretrain 없이 MIM objective와 동시에 학습된다**는 게 핵심 → 기존 BEiT류가 고정 tokenizer(DALL-E VAE 등)를 쓰던 것과 대비
...
*그림은 모두 원 논문에서 가져왔다. Zhou et al., [iBOT](https://arxiv.org/abs/2111.07832), ICLR 2022.*
