...
> 📄 [**U-Net: Convolutional Networks for Biomedical Image Segmentation**](https://arxiv.org/abs/1505.04597) · MICCAI 2015 · Ronneberger, Fischer, Brox

## Abstract

**문제**
딥러닝 segmentation은 라벨된 학습 데이터가 수천 장 필요한데, 의료 영상은 전문가가 라벨링해야 해서 그만큼 모을 수 없음
게다가 세포처럼 **같은 클래스의 인접 객체를 개별로 분리**해야 함

**해결책**
대칭 U자형 encoder-decoder에 **skip connection**을 넣어 다운샘플링에서 잃은 공간 정보를 복원
...
- 단순히 더하는 게 아니라 **조합할 기회를 주는 것**이 요점
...
![Fig. 2](/img/unet/f2.png)

- GPU 메모리 한계로 이미지 전체를 한 번에 처리할 수 없음 → **큰 입력 타일**로 나눠 처리
...
*`f`·`t` 로 시작하는 그림은 원 논문에서 가져왔다. Ronneberger et al., [U-Net](https://arxiv.org/abs/1505.04597), MICCAI 2015.*
