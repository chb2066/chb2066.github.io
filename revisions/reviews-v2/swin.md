...
> 📄 [**Swin Transformer: Hierarchical Vision Transformer using Shifted Windows**](https://arxiv.org/abs/2103.14030) · ICCV 2021 · Liu, Lin, Cao et al.
...
아래가 이 둘을 가능하게 하는 방법론이다.

![Figure 1](/img/swin/f1.png)
...
- **conv layer를 self-attention으로 대체한 시도** — 로컬 윈도우에서 self-attention을 수행해 성능은 올랐지만 **지연 문제**가 생김 → Swin은 shifted window로 해결
...
![Figure 3](/img/swin/f3.png)

**계층적 feature map** — ViT와의 차이는 **위로 갈수록 해상도가 감소하고 채널이 늘어난다**는 점. CNN 백본의 구조를 Transformer로 되살린 것이고, 덕분에 검출·분할처럼 다중 스케일 feature가 필요한 태스크에 바로 붙는다.
...
**Swin Transformer 블록** — 기존 Transformer의 MSA를 **Window MSA와 Shifted Window MSA로 교체**한 구조
...
분류에서의 우위보다 **dense prediction에서의 격차가 훨씬 크다**는 게 중요하다. 계층적 구조와 고해상도 처리 능력이 실제로 그 태스크에서 값을 한다는 뜻이다.
...
*`f`·`t` 로 시작하는 그림은 원 논문에서 가져왔다. Liu et al., [Swin Transformer](https://arxiv.org/abs/2103.14030), ICCV 2021.*
