...
> 📄 [**Exploring Simple Siamese Representation Learning**](https://arxiv.org/abs/2011.10566) · CVPR 2021 · Chen, He
...
**해결책**
그 장치들을 **전부 제거**하고 무엇이 남아야 하는지 확인
...
**붕괴라는 문제**
- 두 뷰의 표현을 가깝게 만들라고만 하면, 모든 입력에 대해 같은 값을 출력하는 것이 **완벽한 해**가 됨
...
이게 중요한 이유는 **stop-gradient 가 최적화를 돕는 보조 장치가 아니라 붕괴를 막는 필수 요소**라는 것을 보여주기 때문이다. 빼면 성능이 조금 나빠지는 게 아니라 학습 자체가 무너진다.
...
논문의 답은 **EMA 가 성능을 조금 올려주기는 하지만 붕괴 방지에는 필요하지 않다**는 것이다. BYOL 에서 EMA 가 필수처럼 보였던 것은, 실제로는 stop-gradient 가 하던 일을 EMA 덕분으로 오해한 면이 있다.

---

## 6. Comparisons

![Figure 3](/img/simsiam/f3.png)

- **6.1** ImageNet linear evaluation 에서 경쟁력 있는 성능. 특히 **100 epoch 같은 짧은 학습에서는 다른 방법들을 앞섬**
- 전이 학습(검출·분할)에서도 경쟁력 유지
- **6.2** SimCLR·SwAV·BYOL 을 **SimSiam 의 특수한 경우**로 배치해 각 방법이 무엇을 더 얹었는지 정리
...
*그림은 모두 원 논문에서 가져왔다. Chen, He, [Exploring Simple Siamese Representation Learning](https://arxiv.org/abs/2011.10566), CVPR 2021.*
