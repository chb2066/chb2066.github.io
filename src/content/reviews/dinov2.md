...
**문제**
자기지도 학습은 라벨 없이 표현을 얻을 수 있지만, 규모를 키울 때 데이터 품질이 걸림돌. 웹에서 긁은 이미지는 소수의 지배적 모드에 쏠려 양이 늘어도 다양성이 따라오지 않음
...
- 기준 데이터가 작으면 → 기준 이미지가 속한 클러스터에서 M개를 샘플링. 소수 데이터를 많이 가져온다는 뜻
- 유사도는 `q·k` 내적으로 구함
...
여기서 눈여겨볼 점은 정제 데이터가 라벨이 아니라 query로 쓰인다는 것이다. 라벨을 쓰지 않으면서도 "무엇이 좋은 데이터인가"의 기준을 정제 데이터가 대신 정해준다.
...
- 가장 큰 ViT-g(1B)를 먼저 학습시켜 고정한 뒤 거기서 작은 모델들로 distill
