...
ResNet이 남긴 것은 특정 아키텍처가 아니라 `y = x + F(x)` 라는 형태 자체다.

이 형태의 값어치는 항등함수가 기본값이 된다는 데 있다. 블록을 추가해도 최소한 나빠지지는 않으므로, 모듈을 안전하게 덧붙일 수 있다. 그리고 `F`를 0으로 초기화하면 사전학습 모델을 건드리지 않고 확장할 수 있다.

LoRA, adapter, ControlNet, adaLN-Zero가 전부 같은 골격이다. "zero-init 잔차 가지로 새 능력을 붙인다"는 패턴은 이 논문에서 시작해서 지금도 계속 재사용되고 있다.

부수적으로 얻은 것도 있다. 잔차 스트림이 각 층이 읽고 쓰는 공유 작업공간이 되면서, 이후 해석가능성 연구에서 개입의 무대가 됐다.
