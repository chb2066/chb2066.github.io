...
- 공통 전제 - 어떤 변형에 대해 불변이어야 하는가를 사람이 정해줘야 함
...
- 이미지 `x` 와 일부를 마스킹한 이미지 내에 `z`(masking token 과 위치정보)를 사용해 원본 이미지를 복원하게 하는 것
- 손실이 입력 공간에 적용된다는 것이 핵심적인 차이
...
- 기존에는 이미지 등을 출력해서 이미지에 대해 손실함수를 적용했다면, 여기서는 출력 벡터를 실제 이미지를 target encoder 에 넣어 나온 진짜 특징 벡터와 비교
- 같은 위치에 해당하는 벡터만 가져와서 비교하고 두 벡터 사이의 L2 distance 를 줄이도록 학습
- 추가 변수 `z`(위치)를 받는 predictor 네트워크를 사용 → `x` 로부터 `z` 에 해당하는 `y` 의 임베딩을 예측
- loss 는 context encoder 의 가중치를 조정하게 함
...
- Context(힌트) - 이미지에서 하나의 큰 사각형 블록을 선정. 크기 0.85~1.0
  ...
  - 따라서 실제 픽셀 양은 0.85 에서 target 이미지를 뺀 0.5~0.6 정도가 남음
...
**차별점**
- SimCLR, BYOL 같은 view-invariance 기반 방법들이 다양한 augmentation 을 사용하는 것과 대비됨
...
- object counting, depth prediction 같은 저수준·국소 태스크에서 평가
...
- 즉 이 방법이 통하려면 "충분히 어렵되 풀 수 있는" 예측 과제로 맞춰야 함

---

## 정리

I-JEPA가 보여준 것은 손실을 어느 공간에 적용하는가가 표현의 성격을 결정한다는 것이다.
