...
**문제**
CNN은 고정된 자원 예산에서 개발되고, 자원이 더 있으면 키워서 성능을 올림. 그런데 어떻게 키울지는 임의로 정해져 왔고, 보통 depth·width·resolution 중 하나만 늘림
...
그런데 더 오래 남은 것은 규칙 자체보다 탐색 비용을 규모에서 분리한 방식이다. 작은 모델에서 한 번 비율을 찾고, 큰 모델에서는 그 비율을 재사용한다. 큰 모델에서 매번 탐색하는 것은 현실적으로 불가능하기 때문이다.
